...
void	put_image(t_data *data, char *img, int y, int x)
{
	...
	mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->img_ptr, x
		* width, y * height);
...
void	show_map(t_data *data)
{
	...
	while (data->map.map[i])
	{
		...
		{
			put_image(data, "textures/dummy.xpm", i, j);
			if (i == 0 || j == 0 || j == data->win.width - 1
				|| i == data->win.height - 1)
				put_image(data, "textures/tree.xpm", i, j);
			else if (data->map.map[i][j] == '1')
				put_image(data, "textures/treewire.xpm", i, j);
			else if (data->map.map[i][j] == 'C')
				put_image(data, "textures/key01.xpm", i, j);
			else if (data->map.map[i][j] == 'P')
				put_image(data, "textures/PB1.xpm", i, j);
			else if (data->map.map[i][j] == 'E')
				put_image(data, "textures/EXIT.xpm", i, j);
			j++;
		}
		i++;
	}
